...
uses dmSCM, dmTDS, System.SysUtils, System.Classes, system.Hash,
...
procedure ReConstructLanes(sl: TStringList; adtFileType: scmDTFileType; ADataSet: TDataSet);
var
  laneValue: Variant;
  s, lane, dtstr: string;
	rt: TDateTime;
...
procedure ReConstructINDV(sl: TStringList; adtFileType: scmDTFileType);
begin
	ReConstructLanes(sl,adtFileType, SCM.qryINDV);
end;

procedure ReConstructTEAM(sl: TStringList; adtFileType: scmDTFileType);
begin
	ReConstructLanes(sl,adtFileType, SCM.qryTEAM);
end;

procedure ReConstructHeat(SessionID, eventNum: integer; gender: string;
	 aEventType: scmEventType; sl: TStringList; adtFileType: scmDTFileType);
var
  HeatNum: integer;
...
begin
	...
	if adtFileType = scmDTFileType.ftUnknown then exit;
  ...
  begin
    sl.Clear; // ensures - only one heat per file.
		HeatNum := SCM.qryHeat.FieldByName('HeatNum').AsInteger;
    // first line - header.
...
procedure ReConstructEvent(SessionID: integer; sl: TStringList; adtFileType: scmDTFileType);
var
i, EventNum: integer;
gender: string;
...
begin
	...
	begin
		EventNum := SCM.qryEvent.FieldByName('EventNum').AsInteger;
		i := SCM.qryEvent.FieldByName('EventID').AsInteger;
...
begin
  ...
  sl := TStringList.Create;
	ReConstructEvent(SessionID, sl, scmDTFileType.ftDO4);
  sl.Free;
end;
...
begin
  ...
  seed := 1;
  sl := TStringList.Create;
	ReConstructEvent(SessionID, sl, scmDTFileType.ftDO3);
  sl.Free;
end;
